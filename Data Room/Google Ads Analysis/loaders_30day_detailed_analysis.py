...
print("=" * 180)
print("LOADERS CAMPAIGNS - COMPLETE 30-DAY ANALYSIS WITH ALL CONVERSIONS")
...
query_campaigns = """
    SELECT
        ...
        campaign.status
    FROM campaign
    WHERE campaign.name LIKE '%Loader%'
...
for row in response_campaigns:
    loaders_campaigns.append({
        'id': row.campaign.id,
        'name': row.campaign.name,
        'status': str(row.campaign.status)
    })

print(f"\n✅ Found {len(loaders_campaigns)} Loaders campaign(s)")

# Analyze each campaign
for campaign in loaders_campaigns:
