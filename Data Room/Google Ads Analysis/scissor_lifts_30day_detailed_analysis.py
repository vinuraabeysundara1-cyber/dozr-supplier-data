...
print("=" * 180)
print("SCISSOR LIFTS CAMPAIGNS - COMPLETE 30-DAY ANALYSIS WITH ALL CONVERSIONS")
...
# Get all Scissor Lift campaigns
query_campaigns = """
    SELECT
        campaign.id,
        campaign.name,
        campaign.status
    FROM campaign
    WHERE campaign.name LIKE '%Scissor%'
...
for row in response_campaigns:
    scissor_campaigns.append({
        'id': row.campaign.id,
        'name': row.campaign.name,
        'status': str(row.campaign.status)
    })

print(f"\n✅ Found {len(scissor_campaigns)} Scissor Lift campaign(s)")

# Analyze each campaign
for campaign in scissor_campaigns:
